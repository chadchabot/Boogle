...
def boogle():
    ...
    cube_list = generate_board(master_cube_list)
    print_board(cube_list)

def generate_board(master_cube_list):
...
def scramble_board(cube_list):
  ...
  for x in range(0, num_cubes):
    next_cube_num = random.randint(0, remaining_cube_count-1)
    cube_count += 1
    next_cube = cube_list.pop(next_cube_num)
